...
from abis.config import DEBUG_PILOT_ABI, BRIDGE_ABI, BRIDGE_OPBNB, OPBNB_CLAIM_NFT_ABI
from utils import logger


class Client:
    # default_abi = DefaultABIs.Token
    default_abi = read_json(DEBUG_PILOT_ABI)
    bridge_abi = read_json(BRIDGE_ABI)
    bridge_opbnb = read_json(BRIDGE_OPBNB)
    opbnb_claim_nft_abi = read_json(OPBNB_CLAIM_NFT_ABI)
    ...
    def get_apprroved(self, contract_address: str, nft_id: str) -> str:
        return (self.w3.eth.contract(
            address=Web3.to_checksum_address(contract_address),
            abi=Client.default_abi
        ).functions.getApproved(nft_id).call())
    ...
    def verif_tx(self, tx_hash) -> bool:
        try:
            ...
            if 'status' in data and data['status'] == 1:
                logger.success(f'{self.address} | transaction was successful: {tx_hash}')
                return True
            else:
    ...
    def approve(self, token_address, spender, nft_id):
